...
- **正文宽度**：工具栏 `− 760 +` 一档一档调正文栏的最大宽度，点中间的数字回到默认 760px。它写回 `markdownAtlas.preview.lineWidth`，所以每个预览、下次重启和导出的 HTML／PDF 都跟着一起变。
- **缩放**、**主题下拉** 也在工具栏里。
...
> 工具栏改主题、调正文宽度、写自定义 CSS 都会写回这个值**本来定义在哪一层**：文件夹设置 → 工作区设置 → 用户设置。所以在 `.vscode/settings.json` 里给某个仓库钉死主题之后，工具栏仍然改得动，改的也是那份工作区设置。
...
| `markdownAtlas.preview.lineWidth` | `760` | 正文栏最大宽度（px）。工具栏的 `宽 − +` 改的就是它 |
